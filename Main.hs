...
takeItem str character
    | str == "true" =   do
    -- logic error? newPlayer = move (extra character)?
...
doChoice2 character = do
                                        ...
                                        input <- getLine
                                        if input == "STATUS" then status character
                                        else if input == "GLOWING LIGHT" then goToLight character
                                        else if input == "STRANGE NOISES" then goToNoises character
                                        else if input == "TAKE A STEP AWAY" then goToStep character
                                        else do
                                                    putStrLn "Command not recognized. Please try again."
                                                    doChoice2 character
                                        
goToLight character = do
                                        ...
                                        if input == "STATUS" then status newPlayer
                                                                                                        ...
                                                                                                        putStrLn "You start poking the bubble with the screwdriver. However, you soon realize that your poking isn't doing anything to the bubble. Why is it even here?! ~.~ Oh well... Guess it's time to try something else.\n"
                                                                                                        goToLight newPlayer
                                        else if input == "SCISSORS" then do
                                                                                                putStrLn "After struggling with the scissors, you finally manage to get out of your entrapement! However, your victory is short lived as you notice that you are inside a maze of a shapeship, filled with winding corridors and thousands of rooms.\n"
                                                                                                goToLevel3 newPlayer
                                        else if input == "CROWBAR" then do
                                                                                                putStrLn "You start attacking the bubble with the crowbar. However, you soon realize that your attacks aren't doing anything to the bubble. Why is it even here?! ~.~ Oh well... Guess it's time to try something else.\n"
                                                                                                goToLight newPlayer
                                        else do
                                                    putStrLn "Command not recognized. Please try again."
                                                    goToLight character

goToNoises character = do
                                        ...
                                        input <- getLine
                                        if input == "STATUS" then status newPlayer
                                        else if input == "BOX" then do
                                                                                        putStrLn "You open the box and find a potion.\nYOU HAVE OBTAINED A POTION!\n"
                                                                                        getBox "true" newPlayer
                                        else if input == "SCREWDRIVER" then do
                                                                                                        putStrLn "After struggling with the screwdriver, you finally manage to get out of your entrapment! However, your victory is short lived as you notice that you are inside a maze of a shapeship, filled with winding corridors and thousands of rooms.\n"
                                                                                                        goToLevel3 newPlayer
                                        else if input == "SCISSORS" then do
                                                                                                putStrLn "You start attacking the door hinges with the scissors. However, you soon realize that your attack isn't doing anything to the door. Why is it even here?! ~.~ Oh well... Guess it's time to try something else.\n"
                                                                                                goToNoises newPlayer
                                        else if input == "CROWBAR" then do
                                                                                                putStrLn "You start attacking the door with the crowbar. However, you soon realize that your attack isn't doing anything to the bubble. Why is it even here?! ~.~ Oh well... Guess it's time to try something else.\n"
                                                                                                goToNoises newPlayer
                                        else do
                                                    putStrLn "Command not recognized. Please try again."
                                                    goToNoises character

getBox str character
    | str == "true" =   do
    --extra (hit (move character))?
                                    let newPlayer = extra (hit character)
                                    putStrLn "You open the box and find a potion.\nYOU HAVE OBTAINED A POTION!\n"
                                    let str = "false"
                                    goToNoises newPlayer
    | str == "false"    =   do
...
goToStep character = do
                                        putStrLn "Ouch! You bump your head on what feels like a metal bar on the ceiling. You didn't realize that the space you were in was so small. You grope around your surroundings in hopes that you can find something that will help you out of this mess. Your hands brush against something that feels like a switch. You brace yourself for the worst and flip it.\nImmediately, the room gets flooded with light.\nYoulook around at your surroundings and realize that you can only take one step in each direction. You start hyperventilating due to your claustrophobia until you notice that there is a tool kit in one of the corners.\nYou open the tool kit, hoping to find something that will help you escape, and find: a SCREWDRIVER, a pair of SCISSORS, and a CROWBAR.\nWhat do you want to grab?\n"
                                        let newPlayer = move character
                                        input <- getLine
                                        if input == "STATUS" then status newPlayer
                                        else if input == "SCREWDRIVER" then do
                                                                                                        putStrLn "You start attacking a wall with the screwdriver. However, you soon realize that your attack isn't doing anything to the door. Why is it even here?! ~.~ Oh well... Guess it's time to try something else.\n"
                                                                                                        goToStep newPlayer
                                        else if input == "SCISSORS" then do
                                                                                                putStrLn "You start attacking a wall with the scissors. However, you soon realize that your attack isn't doing anything to the door. Why is it even here?! ~.~ Oh well... Guess it's time to try something else.\n"
                                                                                                goToStep newPlayer
                                        else if input == "CROWBAR" then do
                                                                                                putStrLn "You start attacking a wall with the crowbar with all of your strength. After some struggle, you finally manage to get out of your entrapment! However, your victory is short lived as you notice that you are inside a maze of a shapeship, filled with winding corridors and thousands of rooms.\n"
                                                                                                goToLevel3 newPlayer
                                        else do
                                                    putStrLn "Command not recognized. Please try again."
                                                    goToNoises character
...
goToGuard str character = do
                                            ...
                                            input <- getLine
                                            if input == "STATUS" then do status newPlayer
                                            else if input == "ITEM" then getHurt "true" newPlayer
                                            else if input == "JUST LEAVE" then do 
                                                        ...
                                                        putStrLn "Command not recognized. Please try again."
                                                        goToGuard str character

getHurt str character
    | str == "true" =   do
    --extra (hit (move character))?
                                    let newPlayer = extra (hit character)
